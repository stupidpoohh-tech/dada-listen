...
1. `/api/health` 를 연다 (로그인 없이 볼 수 있다). `whoami` 항목이
   **404** 면 함수가 없거나 Data API 의 스키마 캐시가 옛것이다.
2. Neon SQL Editor 에 `db/checks/auth_check.sql` 을 통째로 붙여넣고 돌린다.
   함수 존재·반환 타입·실행 권한·`approved` 컬럼을 한 번에 보여주고,
   끝에서 스키마 캐시를 새로 읽게 한다.
